...
module.exports = {
	wrapPayload:function(err, message){
		if (!err)
			...
			return {status:'error', payload:{error:err, message:message}};
	},
				...
				handler.process_internal = function(message, callback){
					...
					if (_this.checkTTL(message)){
						handler.process(message, function(e, result){
							callback(null, _this.wrapPayload(e, result));
						});
					}else{
						callback(null, _this.wrapPayload('TTL limit reached', message));
					}
